...
const ETHTransfer = () => {
  ...
  return (
    <div>
      ...
      <div>
        <form>
          <div className="flex justify-center items-center my-4 text-white text-lg font-semibold">ETH Transfer</div>
        <div className="flex items-center border-2 py-2 px-3 rounded-2xl mb-4 bg-white">
				...
				<input className="pl-2 outline-none border-none bg-white" type="text" value={tx.recipient} onChange={(e) => tx.setrecipient(e.target.value)} placeholder="Recipient" />
      			</div>
				<div className="flex items-center border-2 py-2 px-3 rounded-2xl mb-4 bg-white">
        <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title/><path d="M11.944 17.97L4.58 13.62 11.943 24l7.37-10.38-7.372 4.35h.003zM12.056 0L4.69 12.223l7.365 4.354 7.365-4.35L12.056 0z"/></svg>
					<input className="pl-2 outline-none border-none" type="text" value={tx.amount} onChange={(e) => tx.setamount(e.target.value)} placeholder="Amount (in wei)" />
				</div>
        <div className="flex items-center border-2 py-2 px-3 rounded-2xl mb-4 bg-white">
        <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title/><path d="M11.944 17.97L4.58 13.62 11.943 24l7.37-10.38-7.372 4.35h.003zM12.056 0L4.69 12.223l7.365 4.354 7.365-4.35L12.056 0z"/></svg>
					<input className="pl-2 outline-none border-none" type="text" value={tx.message} onChange={(e) => tx.setmessage(e.target.value)} placeholder="Message (not required)" />
				</div>
        <div className='space-x-3'>
          {App.ethLoading ? (
            <div className='bg-black my-12 py-3 px-4 rounded-full hover:bg-gray-200 hover:cursor-pointer font-bold text-sm'>
              <TailSpin width={28} height={28} color={"black"} />
            </div>

          ) : (
            <a onClick={tx.send} className='bg-white my-12 py-3 px-4 rounded-full hover:bg-gray-200 hover:cursor-pointer font-bold text-sm'>Send</a>
          )}

          {tx.etherscanLink ? (
            <a href={`https://goerli.etherscan.io/address/${tx.recipient}`} target={"_blank"} className='text-white text-xs font-bold'>View Tx</a>
          ): ""}
        </div>
        </form>
      </div>
